...
Returns a value like a `Comparator <https://api.dart.dev/stable/3.4.1/dart-core/Comparator.html>`__ when comparing ``this`` to ``other``.
...
Creates a `Set <https://api.dart.dev/stable/3.4.1/dart-core/Set-class.html>`__ with the same elements and behavior as this ``Set``.
...
often paired with a static ``parse`` function (like `int.parse <https://api.dart.dev/stable/3.4.1/dart-core/int/parse.html>`__).
